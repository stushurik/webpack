...
class HarmonyImportSpecifierDependencyConcatenatedTemplate {
	...
	harmonyInit(dep, source, { runtimeTemplate, dependencyTemplates, moduleGraph }) {
		const module = moduleGraph.getModule(dep);
		const info = this.modulesMap.get(module);
		if (!info) {
	...
	apply(dep, source, { runtimeTemplate: runtime, dependencyTemplates, moduleGraph }) {
		const module = moduleGraph.getModule(dep);
		const info = this.modulesMap.get(module);
		if (!info) {
...
class HarmonyImportSideEffectDependencyConcatenatedTemplate {
	...
	harmonyInit(dep, source, { runtimeTemplate: runtime, dependencyTemplates, moduleGraph }) {
		const module = moduleGraph.getModule(dep);
		const info = this.modulesMap.get(module);
		if (!info) {
			this.originalTemplate.harmonyInit(dep, source, {
