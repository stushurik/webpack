...
HarmonyInitDependency.Template = class HarmonyInitDependencyTemplate {
	apply(dep, source, ctx) {
		const { runtimeTemplate: runtime, dependencyTemplates } = ctx;
		...
		for (const dependency of module.dependencies) {
			...
			if (
				...
				const order = template.getHarmonyInitOrder(dependency, ctx);
				if (!isNaN(order)) {
					list.push({
						order,
		...
		for (const item of list) {
			item.template.harmonyInit(item.dependency, source, ctx);
		}
	}
};
